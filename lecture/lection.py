...
# #
# re.search(pattern, text) -# - находит первое свпадение с шаблоном,
...
#
# re.fullmatch(pattern, text) # - проверяет совпадает ли вся строка указанному шаблону, возращает
# # math-объект в случае совпадения
#
# re.finditer(pattern, text) # делает тоже самое что и fandall но возвращает итерируемый объект,
# # который содержит math-объкты всех совпадений с шаблоном
#
# re.split(pattern, text, maxsplit=1)  # - разбивает строку на части по указанному шаблону, возращает список фрагсментов
#
...
#
# # . ^ $ * + & { } [ ] \ | ( ) - основной набор метасимволов в регулярных выражениях
#
# """
#
...
# \d - цифра, эквивалетно написанию [0-9]. В питоне \в по умолчанию захватывает не только арабские цифры но любые другие
# из Юникода
#
# \D - любой символ кроме цифр
#
# \w - любой буквенно-цифровой символ. Влючает так же символ подчеркивания "_"
#
# \W - антоним \w
#
...
# Внутри символьного класса большинство метасимволов утрачивают свои действия, например шаблон [*+?] означает любой из символов * + ?
# Однако символы -, ^ в символьном  классе имеют особый смысл "-" - это диапазон. "^" - это отрицание, т.е если этот символ находится
# в символьном классе - любой отсуствующий символ в классе, совпадает с шаблоном
# """
# alt entr
# pass
# import re
...
# | - или пример 7|8
# () - групирующие скобки позвалячют сгрупировать или обьединить несколько выражении
...
val = re.sub(pattern, repl="text", string=text, count=0)
print()
